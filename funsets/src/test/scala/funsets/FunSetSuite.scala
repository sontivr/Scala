...
class FunSetSuite extends FunSuite {
  ...
  test("intersect contains all elements common to both sets") {
    new TestSets {
      val s = union(s1, s2)
      val t = union(s1, s3)
      val i = intersect(s, t)
  ...
  test("filter: subset of s for which p holds") {
    ...
    test("forall: all bounded integers within s that satisfy p") {
    new TestSets {
      val s = union(s1, s2)
      val f = forall(s, s2)
      assert(!f, "forall s1")
  ...
  test("exists: returns bounded integer in s that satisfies p") {
    new TestSets {
      val s = union(s1, s2)
      val e = exists(s, s2)
      assert(!e, "exists s1")
  ...
  test("map: returns a set transformed by applying f to each element of s") {
    new TestSets {
      ...
      def f(x: Int): Int = 2 * x
      val m = map(s, f)
      assert(contains(m, 1), "maps to 1")
      assert(!contains(m, 2), "maps to 2")
      assert(contains(m, 4), "maps to 4")
    }
  }
